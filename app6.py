...
app = Flask(__name__)
app.secret_key = "super_secret_key"  # change this to a random string for production

API_KEY = "60efbddb88dd2481343c67fc1140524c"  # ⬅️ paste your key here safely, e.g. "60efbddb88dd2481343c67fc1140524c"
...
def index6():
    ...
    return render_template('index6.html', weather=weather_data, forecast=forecast_data,
                           error=error, unit=unit, history=session.get('history', []))
...
def add_to_history(city):
    """Add a city to the recent search history."""
    history = session.get('history', [])
